...
class GapAligner:
    ...
    def update(self, f_rgb: np.ndarray, f_thr: np.ndarray) -> None:
        ...
        # 1. PCA on combined [f_rgb; f_thr] → D → pca_dim
        pca = PCA(n_components=pca_dim, random_state=42)
        pca.fit(np.concatenate([f_rgb, f_thr], axis=0))   # [2N, D]

        f_rgb_p = pca.transform(f_rgb)   # [N, pca_dim]
        f_thr_p = pca.transform(f_thr)   # [N, pca_dim]

        # 2. Cross-covariance SVD: M = F_rgb_p.T @ F_thr_p  [pca_dim, pca_dim]
        M = f_rgb_p.T @ f_thr_p
        U, S, Vt = np.linalg.svd(M, full_matrices=False)

        # 3. Top-k axes, normalized weights
        k = min(self.k, len(S))
        w = S[:k] / S[:k].sum()

        # 4. Project directions back to original D-dim space
        ...
        self.mean    = torch.tensor(pca.mean_,          dtype=torch.float32)  # [D]
    ...
    def loss(self, f_thr: torch.Tensor, f_rgb: torch.Tensor) -> torch.Tensor:
        ...
        dev   = f_thr.device
        mean  = self.mean.to(dev)        # [D]
        W_t   = self.W_thr.to(dev)       # [D, k]
        W_r   = self.W_rgb.to(dev)       # [D, k]
        sigma = self.sigma_k.to(dev)     # [k]

        # center → project
        pt = (f_thr.float() - mean) @ W_t   # [B, k]
        pr = (f_rgb.float() - mean) @ W_r   # [B, k]
